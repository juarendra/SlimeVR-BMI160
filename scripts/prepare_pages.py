...
def merge_firmware(binaries: list, offsets: dict, output: Path, flash_params: dict) -> Path:
    ...
    cmd = [
        ...
        "merge-bin",
        "-fm",
        flash_params["flash_mode"],
        "-ff",
        flash_params["flash_freq"],
        "-fs",
        flash_params["flash_size"],
        "-o",
        str(output),
    ]

    for binary in binaries:
        cmd.extend([str(binary["offset"]), str(binary["path"])])
...
def main():
    ...
    for key in ["bootloader", "partitions", "firmware"]:
